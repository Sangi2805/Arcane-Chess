...
import { persistBoardViewMode } from "./storage.js";
...
export const setBoardViewModePreference = (mode, { source = "system" } = {}) => {
  ...
  persistBoardViewMode(state.boardViewMode);
...
export const launchSelectedBoard = async ({ trigger = "game-start" } = {}) => {
  ...
  // If 3D is already running and healthy, skip re-initialization
  if (requestedMode === "3d" && validate3DBoardInstance()) {
    console.log("[board] 3D already running and healthy, skipping re-init for trigger:", trigger);
    syncBoard3D({ refreshPerspective: false });
    return "3d";
  }
...
export const handleBoardModeToggle = (mode) => {
  ...
  if (mode === "3d" && state.view !== "game") {
    ...
    state.viewMode = "3D";
    // Do NOT persist 3d when outside a game — causes blank screen on refresh
