...
const registerUser = async ({ email, password, displayName }) => {
  ...
  const existingDisplayName = await User.findOne({
    displayName: { $regex: new RegExp(`^${normalizedInput.displayName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i") }
  }).lean();

  if (existingDisplayName) {
    throw new AuthenticationError("That display name is already taken. Please choose another.", 409);
  }
